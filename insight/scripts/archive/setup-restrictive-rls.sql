...
-- 6. Crear políticas restrictivas

-- Política base restrictiva que deniega todo por defecto
CREATE POLICY "deny_all" ON public.users
    AS RESTRICTIVE
    FOR ALL
    TO public
    USING (false)
...
CREATE POLICY "allow_insert_paciente_unauth" ON public.users
    AS RESTRICTIVE
    FOR INSERT 
    TO public
    WITH CHECK (
...
CREATE POLICY "allow_admin_insert_any" ON public.users
    AS RESTRICTIVE
    FOR INSERT 
    TO authenticated
    WITH CHECK (
